...
const KlarnaCheckout = ({ product, showFormSelect, user }) => {
    ...
    >
      ...
      <Box>
        <Box className=" !h-fit mt-auto flex items-center flex-row">
          <IconButton disabled={authorizing} onClick={() => showFormSelect(false)} className="mr-2 font-semibold aspect-square text-[16px] md:text[18px] rounded-full " color="buttonGreen">
            <NavigateBeforeRoundedIcon fontSize="large" />
          </IconButton>

          <LoadingButton loadingPosition="end" loading={authorizing} color="buttonGreen" variant="contained" sx={{ mt: 0, width: "100%" }} size="large" onClick={() => authorizePayment()}>
            Paga
          </LoadingButton>
        </Box>
