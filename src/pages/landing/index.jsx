...
//styles -use more css reduces rendering time
...
import { CaKlarna } from "../../../components/Icons";
...
const Timer = ({ date, days }) => {
  ...
  return (
    <div className="flex flex-row items-center gap-4 lg:gap-[25px] w-full justify-center">
...
const InformationBox = ({ children, Prompt, index, indexText }) => {
  return (
    <div className={cn(s.ibRoot)}>
      <div className={s.content}>
        <div
          className={cn(
            "lg:w-[51px] lg:min-w-[51px] lg:max-w-[51px] lg:h-[51px] lg:text-[32px] flex items-center justify-center rounded-[50%] border-[#ffffff] border-solid border-[3px] text-white",
            s.index
          )}
        >
          {index}
        ...
        <p
          className={cn(
            "font-semibold text-[white] text-left leading-[128%]",
            s.indexText
          )}
        >
          {indexText}
      ...
      <div className={cn("relative rounded-[9px] w-full", s.overlayContent)}>
        <CaBackground
          style={{ width: "100%" }}
...
const LandingPaymentPage = ({ data, startSession }) => {
  ...
  const priceItemStyle = {
    ...
    ["@media (min-width:1180px)"]: {
      gridTemplateColumns: "8fr 0.8fr 5fr",
    },
  ...
  React.useEffect(() => {
    if (product) {
      const originalPrice = {
        due_amount: (Number(product.price) / 100)
          .toFixed(2)
          .toString()
          .padStart(2, "0"),
        original: (Number(product.original_price) / 100)
          .toFixed(2)
          .toString()
          .padStart(2, "0"),
        no_iva: (Number(product.original_price_no_iva) / 100)
          .toFixed(2)
          .toString()
          .padStart(2, "0"),
        remaining: (
        ...
        klarna_installments: (
          Math.floor(
            (Number(product.original_price) - Number(product.price)) / 3
          ) / 100
        )
          .toFixed(2)
          .toString()
      ...
      //convert to string with 2 decimal places separated by a coma
      originalPrice.due_amount = originalPrice.due_amount.replace(".", ",");
      originalPrice.original = originalPrice.original.replace(".", ",");
      originalPrice.no_iva = originalPrice.no_iva.replace(".", ",");
      originalPrice.remaining = originalPrice.remaining.replace(".", ",");
      originalPrice.klarna_installments =
        originalPrice.klarna_installments.replace(".", ",");
  ...
  return (
    <>
      <div className="h-full overflow-x-hidden bg-edu-100">
        ...
        <section className="text-center ">
          ...
          <Box
            ...
            className={cn(
              "gap-y-4 relative gap-x-[min(3.5rem,5vw)] w-full px-[min(2rem,4vw)] items-center mx-auto max-w-[1500px] pb-6"
            )}
          >
            <InformationBox
              index={1}
            ...
            <InformationBox
              ...
              Prompt={() => (
                <>
                  {`Pagabile in 3 rate da \n €${price?.klarna_installments} con\n `}
                  <CaKlarna className="mx-auto" />
                </>
              )}
            >
              <Box
              ...
              >
                ...
                <Box sx={priceItemStyle}>
                  <p>Meno iscrizione:</p>
                  <b className="pl-[10px]">-</b>
                  <b className="relative">
                    €{price?.due_amount}
                    {/* <div className=" lg:max-w-[105px] w-[40%] max-w-[160px] lg:w-full absolute my-auto h-[3px] !bg-[#861313] " /> */}
                  </b>
                </Box>
          ...
          <section className="pt-14 text-center  bg-white pb-6">
            <h1 className="py-5 lg:mb-4  font-semibold leading-none text-2xl lg:text-[44px] 3xl:text-4xl max:text-6xl text-edu-900">
              Riguarda la presentazione del percorso
            </h1>
            <div className="w-full bg-white bg-top bg-no-repeat bg-cover min-h-[300px] flex flex-col items-center overflow-x-hidden">
              <div className="">
          ...
          <p className="mb-14 mt-14  font-semibold leading-none text-2xl lg:text-[44px] 3xl:text-4xl max:text-6xl text-edu-900">
