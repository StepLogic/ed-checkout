...
const StripeCheckout = ({ userToken, product, iva, userInfo }) => {
  ...
  const savePayment = async (paymentIntentId) => {
    if (path.indexOf("new-subscriber") > -1 || path.indexOf("new-user") > -1) {
      ...
      if (data) {
        navigate("/thank-you", { replace: true, state: { iva, existing_user: false, token: data?.user?.token } });
      }
  ...
  return (
    <>
      ...
      >
        <TextField className="w-full" placeholder="Nome titolare carta" name="nome" onChange={(e) => formik.setFieldValue(e.target.name, e.target.value?.strCapitalization())} onBlur={formik.handleBlur} type="text" value={formik.values.nome} error={formik.errors.nome && formik.touched.nome} helperText={formik.touched.nome && formik.errors.nome} />
        <TextField className="w-full" placeholder="Cognome titolare carta" name="cognome" type="text" onBlur={formik.handleBlur} onChange={(e) => formik.setFieldValue(e.target.name, e.target.value?.strCapitalization())} value={formik.values.cognome} error={formik.errors.cognome && formik.touched.cognome} helperText={formik.touched.cognome && formik.errors.cognome} />
